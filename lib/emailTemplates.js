import { OWNER_EMAIL } from "./sendEmail";
...
export function bookingOwnerEmail({ name, phone, email, social, date, eventType, location, startTime, hours }) {
  const content = `
    <div style="background-color: #f8f8f8; border-radius: 12px; padding: 24px;">
      ...
      ${dataTable([
        ...
        row("Mesto održavanja", escapeHtml(location)),
...
export function bookingConfirmationEmail({ name, phone, email, social, date, eventType, location, startTime, hours }) {
  const content = `
    <div style="background-color: #f8f8f8; border-radius: 12px; padding: 24px; margin-bottom: 24px;">
      <p style="margin: 0 0 16px; font-size: 12px; letter-spacing: 0.15em; text-transform: uppercase; color: #D4AF37; font-family: Arial, Helvetica, sans-serif; font-weight: 600;">
        Pregled vašeg upita
    ...
    <div style="background-color: #111111; border-radius: 12px; padding: 20px 24px; text-align: center;">
      <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #cccccc; font-family: Arial, Helvetica, sans-serif;">
        Uskoro će vas neko iz našeg tima kontaktirati radi potvrde termina.
      </p>
    </div>
  `;
  ...
  return emailLayout({
    ...
    intro:
      "Uspešno smo primili vaš upit za rezervaciju 360° video booth usluge. Uskoro će vas neko iz našeg tima kontaktirati radi potvrde termina i finalnih detalja.",
    content,
